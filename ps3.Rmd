...
The results are pretty much what I expected. All three standard errors are roughly similar. The robust standard errors are larger than the conventional OLS estimators which is expected. The bootstrap method is right in the middle of the other two methods in terms of efficiency.
...
confidence_interval <- function(b, b_se){
  return((b - beta_conventional_full)/b_se < 1.96)
}

problem1c <- function(n){
  mc_sample <- ak_full[sample(1:nrow(ak_full), n, replace = T),]
